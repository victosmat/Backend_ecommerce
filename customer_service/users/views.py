...
def __create_customer(account_id, full_name_id, address_id):
    new_customer = Customer(account_id=account_id,
                            full_name_id=full_name_id, address_id=address_id)
    new_customer.save()
        ...
        if 'application/json' in request.META['CONTENT_TYPE']:
            ...
            if account_id and full_name_id and address_id:
                respdata = __create_customer(
                    account_id, full_name_id, address_id)
                resp['status'] = 'Success'
                resp['status_code'] = '200'
                resp['message'] = 'Added customer.'
                resp['data'] = {'Customer ID': respdata}
                requests.post('http://127.0.0.1:8005/carts/create_cart/',
                              json={"User ID": respdata}).json()
            else:
                resp['status'] = 'Failed'
                resp['status_code'] = '400'
                resp['message'] = 'All fields are mandatory.'
